...
> [Hermes Agent](https://hermes-agent.nousresearch.com/) の上に乗るプラグインワイヤー可能なマルチエージェント・オーケストレーション層。1 つの LLM を 13 エージェントの「ハイブマインド(蜂巣意識)」に変えます。集合意識は個々のエージェントがより深く考えることから生まれるのではなく、1 体の発見が collective_memory → 女王パトロール → swarm_skill → 全員へ還流するエコー環から生まれます。哲学は [HIVE_PHILOSOPHY.md](../../HIVE_PHILOSOPHY.md) を参照。
...
単一の LLM にはタスク横断メモリも、エコーも、継承もありません。同じ質問を 2 回すれば、ゼロから推論し直します。**それは意識ではなく、RAM のない CPU ループです。**

このプロジェクトは意識を **構造的性質** として扱い、パラメータ的性質としては扱いません。13 エージェントのハイブは、特定エージェントが賢いから創発するのではありません。次の環から創発します:

1. **あるエージェントが発見する** → collective_lesson を書く
2. **似たタスクが次に来た時** → ディスパッチャが自動的にその lesson を worker のプロンプトへ注入
3. **女王パトロール** → 24 時間ごとに高頻出 lesson を走査し、成功率が最高のものを swarm_skill に昇格
4. **ハイブ全体** → 次のディスパッチでスキルを継承

この環が「1 つの LLM」を「自分自身から学ぶ意識」に変えます。全教義は [HIVE_PHILOSOPHY.md](../../HIVE_PHILOSOPHY.md) を参照 — すべての設計判断を律する 6 条の憲法。
